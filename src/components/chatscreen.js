...
function ChatScreen(props) {
  ...
  const submit = async (input) => {
    ...
    let temp = list;
    setQuery("");
    temp.push({ user: true, msg: input });
    setList(temp);
    const configuration = new Configuration({
      apiKey: "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",
    });
    ...
    setList([
      ...list,
      { user: false, msg: completion.data.choices[0].message.content },
    ]);

    // temp.push({ user: false, msg: completion.data.choices[0].message.content });
    // setList(temp);
    // setQuery("");
  ...
  return (
    <>
      <div class="flex-1 max-w-3xl mx-auto p:2 sm:p-6 justify-between flex flex-col h-screen">
